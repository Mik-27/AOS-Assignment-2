...
void evict_page_to_disk(struct proc* p) {
    ...
    for(int i=0; i < MAXHEAP; i++){
        if(p->heap_tracker[i].loaded == 0 && p->heap_tracker[i].startblock == -1 && p->heap_tracker[i].addr != 0xFFFFFFFFFFFFFFFF){
            if(idx == -1) {
                idx = i;
            } else if (p->heap_tracker[i].last_load_time < p->heap_tracker[idx].last_load_time) {
                idx = i;
            }
        }
    }
...
void retrieve_page_from_disk(struct proc* p, uint64 uvaddr) {
    ...
    /* Print statement. */
    print_retrieve_page(uvaddr, p->heap_tracker[page_idx].startblock);
...
heap_handle:
    ...
    uint64 sz_heap = uvmalloc(p->pagetable, faulting_addr, faulting_addr + PGSIZE, PTE_W);
